...
def make_statistics(arg_in): 
    ...
    X=energy[:, 1:2] # distance
    Y=energy[:, 4:5] # totalEnergy
    Z=energy[:, 3:4] # packet Sent
    W=energy[:, 2:3] # packet Received

    titlex="Consumed energy vs distance in meters (simTime = "+str(simTime[0])+ " seconds)"
    
    plt.xlabel('distance (m)')
    plt.ylabel('Energy(J) and sent/received packets')
    plt.title(titlex)
    plt.plot(X, Y, label='Total Energy Consumed By WiFi Node 1 (destiny) in Joules')
    plt.plot(X, Z, label='Amount of packets sent')
    plt.plot(X, W, label='Amount of packets received')
    #plt.plot(X, Z, '-r')
    ...
    #print("OK")
    #aux1="Simulation time in seconds="+str(simTime)
    ...
    plt.legend(loc='best', shadow=True, fancybox=True,fontsize='small')
    plt.show()

if __name__ == "__main__":
   #inputfile =  '/home/ubu01/workspace/bake/source/ns-3.29/results/ns3e.output'
   outputfile = '/home/ubu01/repos/ns-3-allinone/ns-3-dev/results/data.log'

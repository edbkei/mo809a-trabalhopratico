...
NS_LOG_COMPONENT_DEFINE ("trabprat");
...
main (int argc, char *argv[])
{
  ...
  double Prss = -80; 
  uint32_t PacketSize = 200;   // bytes, default 200
  bool verbose = false;
  ...
  wifi.SetStandard (WIFI_PHY_STANDARD_80211b); //WIFI_PHY_STANDARD_80211b
  ...
  wifiChannel.AddPropagationLoss ("ns3::FriisPropagationLossModel"); // DEFAULT FriisPropagationLossModel
                                 //   "Exponent", DoubleValue(3.0));  // only ofdmrate12mbps
  ...
  radioEnergyHelper.Set ("TxCurrentA", DoubleValue (0.0174)); // default 0.0174
  radioEnergyHelper.Set ("RxCurrentA", DoubleValue (0.0394)); // default 0.0197
  ...
  //Ptr<Socket> source2 = Socket::CreateSocket (networkNodes.Get (2), tid);    // node 2, Source
  ...
  // energy source
  Ptr<BasicEnergySource> basicSourcePtr = DynamicCast<BasicEnergySource> (sources.Get (1)); // (1)
  ...
  // energy harvester
  Ptr<BasicEnergyHarvester> basicHarvesterPtr = DynamicCast<BasicEnergyHarvester> (harvesters.Get (1)); //(1)
  ...
  //AnimationInterface anim ("trabprat.xml");
  Simulator::Run ();
